...
#ifdef USE_LEMON
void lemonBenchmark(GR *cgr, CapMap *ccap,
...
#ifdef USE_LEDA
void ledaBenchmark(GR *gr, CapMap *cap,
                   GR::Node source, GR::Node target)
{
...
#else
  if (flow_value < 0) std::cout << "\tError" << std::flush;
  std::cout << '\t' << timer.realTime() << std::flush;
#endif
}
#endif
