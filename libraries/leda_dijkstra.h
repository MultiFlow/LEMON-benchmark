...
class dijkstra {
  ...
  node opposite(
      const graph_t& G,
      edge           e,
      node s) { // if (graph::category() == bidirectional_graph_category ||
 ...
 public:
...
#if defined(LEDA_STD_HEADERS) || defined(__mipspro__) || defined(_MSC_VER) || \
    defined(__DMC__)
  ...
  double
  sum_of_distances(const graph_t& G, dist_array& dist, pred_array& pred) {
    double D = 0;
