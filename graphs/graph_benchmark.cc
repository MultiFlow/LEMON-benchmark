 ...
 public:
  ...
  void operator()() {
    for (typename Graph::ArcIt e(_gr); e != INVALID; ++e)
      _code =
          _code * P ^ _gr.id(e) ^ _gr.id(_gr.source(e)) ^ _gr.id(_gr.target(e));
  }
  int result() { return _code; }
};
...
void benchmarkGraph(const GR& gr, int n, int m) {
  ...
  cout << "|               | Iterator 1       | Iterator 2       |" << endl;
  cout << "|---------------|-------------|-------------|" << endl;
  ...
  if (ni1.result() != ni2.result())
    exit(-1);
  if (ai1.result() != ai2.result())
    exit(-1);
  if (oai1.result() != oai2.result())
    exit(-1);
  if (iai1.result() != iai2.result())
    exit(-1);
  if (oiai1.result() != oiai2.result())
    exit(-1);
}
...
int main(int argc, char* argv[]) {
  ...
  cout << "Benchmark tests on a random digraph (n = " << n << ", m = " << m
       << ")" << endl
       << endl;
  ...
  // Iteration test
  cout << "## Iteration (per graph type)" << endl;

  const std::string prefix = "### ";
#ifdef LIST1
  cout << prefix + "ListDigraph" << endl;
  benchmarkGraph(lgr1, n, m);
#endif

#ifdef LIST2
  cout << prefix + "ListDigraph (sorted arcs)" << endl;
  benchmarkGraph(lgr2, n, m);
#endif

#ifdef SMART1
  cout << prefix + "SmartDigraph" << endl;
  benchmarkGraph(sgr1, n, m);
#endif

#ifdef SMART2
  cout << prefix + "SmartDigraph (sorted arcs)" << endl;
  benchmarkGraph(sgr2, n, m);
#endif

#ifdef STATIC
  cout << prefix + "StaticDigraph" << endl;
